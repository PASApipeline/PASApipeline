...
main: {
    ...
    foreach my $asmbl_id (sort keys %$contig_to_gene_list_href) {
        ...
        foreach my $gene_id (@gene_ids) {
            ...
            $asmbl_id =~ /(S\d+)_(asmbl_\d+)/ or die "Error, cannot extract pasa asmbl info from orf: " . $gene_obj_ref->toString();
            
            my $subcluster = $1;
            my $pasa_asmbl = $2;

            my $transcript_struct = $cdna_acc_to_transcript_structure{$pasa_asmbl} or die "Error, no cdna struct for $pasa_asmbl";
            ...
            eval {
                ...
                if ($new_orf_gene) {
                    ...
                    my $orf_count = $orf_counter{$pasa_asmbl}++;
                    $new_orf_gene->{asmbl_id} = $transcript_struct->{contig};
                    $new_orf_gene->{TU_feat_name} = "t.$pasa_asmbl.$orf_count";
                    $new_orf_gene->{Model_feat_name} = "m.$pasa_asmbl.$orf_count";
                    $new_orf_gene->{com_name} = "PASA ORF";
                    
                    print $new_orf_gene->to_GFF3_format() . "\n";
...
sub parse_transcript_alignment_info {
    ...
    while (<$fh>) {
        ...
        $info =~ /ID=(S\d+)-(asmbl_\d+)/ or die "Error, cannot parse PASA info from $info";
        my $subcluster = $1;
        my $asmbl = $2;
...
sub parse_cdna_seq_lengths {
    ...
    while (my $seq_obj = $fasta_reader->next()) {
        ...
        $acc =~ /(asmbl_\d+)/ or die "Error, cannot parse pasa asmbl from $acc";
        my $asmbl = $1;
