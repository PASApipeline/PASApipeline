...
main: {
    ...
    my $blast_outfile = join (".", "blast", $Evalue, $num_hits, "outfmt6");
	...
	unless (-s $blast_outfile) {
		...
		if ($prot_mode) {
			unless (-s "$fasta_file.pin") {
				...
				my $cmd = "makeblastdb -in $fasta_file -dbtype prot";
                &process_cmd($cmd);
			}
			
			#my $cmd = "blastall -p blastp -d $fasta_file -i $fasta_file -m 8 -e $Evalue -v $num_hits -b $num_hits -F \'m S\' > $blast_outfile";
			my $cmd = "blastp -query $fasta_file -db $fasta_file -evalue $Evalue -outfmt 6 -max_target_seqs $num_hits -num_threads $num_threads > $blast_outfile";
            
            &process_cmd($cmd) unless (-s $blast_outfile);
			...
			unless (-s "$fasta_file.nin") {
				...
				my $cmd = "makeblastdb -in $fasta_file -dbtype nucl";
                &process_cmd($cmd);
			}
			
            #my $cmd = "$prog -d $fasta_file -i $fasta_file -m 8 -e $Evalue -v $num_hits -b $num_hits -F \'m D\' > $blast_outfile";
			
            my $cmd = "blastn -query $fasta_file -db $fasta_file -evalue $Evalue -outfmt 6 -max_target_seqs $num_hits -num_threads $num_threads > $blast_outfile";
