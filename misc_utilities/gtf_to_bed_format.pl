...
main: {
	...
	while (<$fh>) {
        ...
        $name_info{$trans_id} = "$gene_id|$trans_id";
        if (my $gene_name = $atts{gene_name}) {
            $name_info{$trans_id} .= "|$gene_name";
        }
        if (my $is_linc = $atts{LINC}) {
            $name_info{$trans_id} .= "|LINC";
        }
	...
	foreach my $scaff (sort keys %genome_trans_to_coords) {
		...
		foreach my $gene_id (keys %$genes_href) {
			...
			foreach my $trans_id (keys %$trans_href) {
                ...
                my $name = $name_info{$trans_id};

				$gene_obj->{TU_feat_name} = $gene_id;
				$gene_obj->{Model_feat_name} = $trans_id;
				$gene_obj->{com_name} = ($name) ? $name : "$gene_id $trans_id";
