...
foreach my $asmbl_id (sort keys %$contig_to_gene_list_href) {
    ...
    foreach my $gene_id (@gene_ids) {
		...
		foreach my $isoform ($gene_obj_ref, $gene_obj_ref->get_additional_isoforms()) {
            ...
            my ($exon_lend, $exon_rend) = (1, length($genome_seq));
            my ($exon_end5, $exon_end3) = ($isoform->get_orientation() eq '+') ? ($exon_lend, $exon_rend) : ($exon_rend, $exon_lend);

            my ($exon_obj) = $isoform->get_exons();
            $exon_obj->set_coords($exon_end5, $exon_end3);
			...
			if ($utr_seq 
                && length($utr_seq) >= 3 #not just a partial codon 
                ) {
